...
$$
\begin{align*}
&\langle F_i^v (\boldsymbol{x}, t) F_j^v (\boldsymbol{x}', t') \rangle = \\
...
**Helical turbulence.**
We consider developed turbulence with helicity, which from a point of view of $\boldsymbol{F}^v$ means, for instance, turbulence in a rotating medium, generated to sense differences in angular velocity directions. The existence of nonzero helicity imposes additional conditions on the energy source of incompressible fluid flow, i.e. to the correlator $\mathrm{D}_{i j}^v$.
...
$$
\begin{align*}
&\langle v_i v_j \rangle &= \frac{\beta \beta^*}{\xi \xi^*} \mathrm{\hat D}^v R_{ij}, \\
