...
This implies the non-vanishing of the helicity $\langle \boldsymbol{v} \cdot\text{curl }\boldsymbol{v} \rangle$, i.e. existense of the correlation between velocity $\boldsymbol{v}$ and vorticity $\text{curl }\boldsymbol{v}$. Thus, we consider a model with a less rich group of global symmetries than the Galilean group $\mathbb{G}(1,3)$ (here we mean precisely three-dimensional case). In our case, the parity symmetry $\mathbb{Z}_2^P \equiv \left\{ \mathbb{1}_3, P_3 \right\} = \mathbb{O}(3)/\mathbb{SO}(3)$, is somehow violated.
...
\Delta =
\begin{bmatrix}
-\mathrm{K}_{12}^{-\mathrm{T}}\mathrm{K}_{22}\mathrm{K}_{12}^{-1} & \mathrm{K}_{12}^{-\mathrm{T}}\\
