...
def compare_fun(tup1, tup2):  # it gives the one that is bigger if the smaller one is found in it, or 0 if it is not found
...
for i in range(len(internal_lines)-1):
    ...
    [loop.append(x) for x in pomoc]  # All possible combinations of propagators for any loops

i = 0
while i < len(loop):  # Check if the given lines (propagators) combination is a loop
    slucka = [vnutorne[in1][0] for in1 in loop[i]]  # prida propagator zodpovedajuci tuple
    hodnoty = list(itertools.chain.from_iterable(slucka))  # prepise dvojice do list vertexov
    sucet = list(map(lambda x: hodnoty.count(x), range(max(hodnoty) + 1 )))  # srata kolko krat sa dany vertex nachadza v slucke
...
Fey_graphs.write("\n"+'Loops in the diagram for a given internal momentum (digit coresponds the line): '+ str(loop) + "\n")
...
for i in loop: 
    ...
    for j in hybnost:
        if j in i:
            ...
            prop[2 * zaciatok + 1] = -1
            prop[2 * zaciatok] = -1
            while prop.count(-1) < len(prop): # I go through the loop, I follow the propagator with D_v from vertex with smaller number to bigger according to the indexes, if it fits, it goes (+), the opposite (-)
                poradie = prop.index(hodnota) # I write the momentum into the propagator in the tuple
                if poradie%2 == 0:
...
P_structure = []  # I save the structures so that I don't have to guess through all possible combinations (faster running of the program) [ [momentum, index 1, index 2]]
H_structure = []  # I save the helical structures so that I don't have to guess through all possible combinations (faster running of the program) [ [momentum, index 1, index 2]]
...
for i in internal_lines: # I write into the Tenzor, the projection part of the propagators (vv, Vv = v'v, Bb = b'b )
    ...
    in2 = indexy.index(i)
    if linia[1] == ['v', 'v']:
        Tenzor = Tenzor * ( P(hybnost[i], in1, in2) + I * rho * H(hybnost[i], in1, in2) ) # H_{ij} (k) = \epsilon_{i, j, l} k_l/ k = H(k, i, j) - it is part with levi-civita symbol and momentum 
        ...
        Tenzor = Tenzor * ( P(hybnost[i], in1, in2)) # Here I can define new propagator for tenzor structure: vb, bv, ...
        integral = integral + 'PBb['+ str(hybnost[i]) + ']*'
    elif linia[1] == ['v', 'b']:
...
Fey_graphs.write("\n"+'Momentum in propagators for the Wolfram Mathematica file: '+ str(integral[:-1]) + "\n")


kd_structure = []  # I save the kronecker delta so that I don't have to guess through all possible combinations (faster running of the program) [ [index 1, index 2]] ... kd(index 1, index 2)
hyb_structure = []  # I save the momemntum and their index (faster running of the program) [ [ k, i] ] ... k_i = hyb(k, i) 
polia = list(map(lambda x: x[1] ,moznost)) # polia - the ordered list: [ V, v, v, b, V, v,... ] - the first three fields corespond the 0 vertex 
for i in range(number_int_vert): # part to add vertices - all vertieces have 
    ...
    if vrchol_usp == ['B', 'b', 'v']: # it finds the indexes of fields and add  to the Tenzor
        in1 = 3*i + vrchol.index('B')
        in2 = 3*i + vrchol.index('b')
        in3 = 3*i + vrchol.index('v')
        Bbv = I*(hyb(moznost[in1][2], in3) * kd(in1, in2) - A * hyb(moznost[in1][2], in2)* kd(in1, in3))
        Tenzor = Tenzor*Bbv
        kd_structure.append([in1, in2]) # it remembers structures for kronecker delta
        kd_structure.append([in1, in3])
        hyb_structure.append([moznost[in1][2], in3]) # it remembers structures for momentum
        hyb_structure.append([moznost[in1][2], in2])
    elif vrchol_usp == ['V', 'v', 'v']:
        in1 = 3*i + vrchol.index('V')
        in2 = [3*i, 3*i + 1, 3*i + 2]
        in2.remove(in1)
        Vvv = I*(hyb(moznost[in1][2], in2[0]) * kd(in1, in2[1]) + hyb(moznost[in1][2], in2[1])* kd(in1, in2[0]))
        Tenzor = Tenzor*Vvv
...
Fey_graphs.write("\n"+'The tensor structure of the diagram after calculation: '+ str(Tenzor) + "\n")
...
for in2 in kd_structure:
    structurep = list()
    for in1 in P_structure:  # calculation via Kronecker's delta function: P(k, i, j) kd(i, l) = P(k, l, j)
        if in1[1] == in2[0]:
            Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), P(in1[0], in2[1], in1[2]))
            structurep.append([in1[0], in2[1], in1[2]])
        elif in1[1] == in2[1]:
            Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), P(in1[0], in2[0], in1[2]))
            structurep.append([in1[0], in2[0], in1[2]])
        elif in1[2] == in2[0]:
            Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), P(in1[0], in1[1], in2[1]))
            structurep.append([in1[0], in1[1], in2[1]])
        elif in1[2] == in2[1]:
            Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), P(in1[0], in1[1], in2[0]))
        ...
        if Tenzor.coeff(kd(in2[0], in2[1])) == 0:
            ...
            break
    P_structure = P_structure + structurep  # it adds all newly created structures to the list
    structureh = list()
    for in1 in H_structure:  # calculation via Kronecker's delta function: H(k, i, j) kd(i, l) = H(k, l, j)
        if in1[1] == in2[0]:
            Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), H(in1[0], in2[1], in1[2]))
            structureh.append([in1[0], in2[1], in1[2]])
        elif in1[1] == in2[1]:
            Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), H(in1[0], in2[0], in1[2]))
            structureh.append([in1[0], in2[0], in1[2]])
        elif in1[2] == in2[0]:
            Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), H(in1[0], in1[1], in2[1]))
            structureh.append([in1[0], in1[1], in2[1]])
        elif in1[2] == in2[1]:
            Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*kd(in2[0], in2[1]), H(in1[0], in1[1], in2[0]))
...
while i < len(P_structure):  # discard from the Tensor structure what is zero for the projection operator P_ij (k) * k_i = 0
    ...
    if Tenzor.coeff(P(in1[0], in1[1], in1[2])) == 0:
        ...
        if in1[0] == - k or in1[0] == - q: # Replace in the tensor structure in the projection operators:  P(-k,i,j) = P(k,i,j)
...
while len(H_structure) > i:  # sipmplify in the Tenzor part H_{ij} (k) P_{il} (k) =  H_{il} (k)
    ...
    for in2 in P_structure:
        if in1[0] == in2[0] and Tenzor.coeff(H(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2])) != 0:
            if in1[1] == in2[1]:
                Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), H(in1[0], in2[2], in1[2]))
                H_structure += [[in1[0], in2[2], in1[2]]]
            elif in1[1] == in2[2]:
                Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), H(in1[0], in2[1], in1[2]))
                H_structure += [[in1[0], in2[1], in1[2]]]
            elif in1[2] == in2[1]:
                Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), H(in1[0], in1[1], in2[2]))
                H_structure += [[in1[0], in1[1], in2[2]]]
            elif in1[2] == in2[2]:
                Tenzor = Tenzor.subs(H(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), H(in1[0], in1[1], in2[1]))
...
while len(P_structure) > i:  # sipmplify in the Tenzor part  P_{ij} (k) P_{il} (k) =  P_{il} (k)
    ...
    for j in range(i+1, len(P_structure)):
        ...
        if in1[0] == in2[0] and Tenzor.coeff(P(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2])) != 0:
            if in1[1] == in2[1]:
                Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), P(in1[0], in2[2], in1[2]))
                structurep.append([in1[0], in2[2], in1[2]])
            elif in1[1] == in2[2]:
                Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), P(in1[0], in2[1], in1[2]))
                structurep.append([in1[0], in2[1], in1[2]])
            elif in1[2] == in2[1]:
                Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), P(in1[0], in1[1], in2[2]))
                structurep.append([in1[0], in1[1], in2[2]])
            elif in1[2] == in2[2]:
                Tenzor = Tenzor.subs(P(in1[0], in1[1], in1[2])*P(in2[0], in2[1], in2[2]), P(in1[0], in1[1], in2[1]))
                structurep.append([in1[0], in1[1], in2[1]])
    if Tenzor.coeff(P(in1[0], in1[1], in1[2])) == 0:
        P_structure.remove(in1)
    else:
        i +=1
    P_structure = P_structure + structurep  # it add all newly created structures to the list
...
for i in P_structure:  # Define transverse projection operator P(k,i,j) = kd(i,j) - hyb(k,i)*hyb(k,j)/k^2
    k_c = i[0].coeff(k)
    q_c = i[0].coeff(q)
    Tenzor = Tenzor.subs(P(i[0],i[1],i[2]), kd(i[1],i[2]) - (k_c*hyb(k,i[1]) + q_c*hyb(q,i[1]))*(k_c*hyb(k,i[2]) + q_c*hyb(q,i[2]))/(k_c**2*k**2 + q_c**2*q**2 + 2*k_c*q_c*k*q*z))
...
while inkd == 0:   # calculation part connected with the kronecker delta function: kd(i,j) *hyb(k,i) = hyb(k,j)
    for in1 in kd_structure:  # beware, I not treat the case if there remains a delta function with indexes of external fields !!
...
while len(H_structure) > i: # calculation for helical term
    in1 = H_structure[i] 
    clen = Tenzor.coeff(H(in1[0], in1[1], in1[2]))
    if clen.coeff(hyb(in1[0], in1[1])) != 0:  # I throw out the part:  H (k,i,j) hyb(k,i) = 0
        Tenzor = Tenzor.subs( H(in1[0], in1[1], in1[2]) * hyb(in1[0], in1[1]), 0)
    if clen.coeff(hyb(in1[0], in1[2])) != 0:
        Tenzor = Tenzor.subs( H(in1[0], in1[1], in1[2]) * hyb(in1[0], in1[2]), 0)
    if in1[0] == k and clen.coeff( hyb(q, in1[1])* hyb(q, in1[2])) != 0:  # I throw out the part:  H (k,i,j) hyb(q,i) hyb(q, j) = 0
        Tenzor = Tenzor.subs( H(in1[0], in1[1], in1[2]) * hyb(q, in1[1]) * hyb(q, in1[2]), 0)
    if in1[0] == q and clen.coeff( hyb(k, in1[1])* hyb(k, in1[2])) != 0:
        Tenzor = Tenzor.subs( H(in1[0], in1[1], in1[2]) * hyb(k, in1[1]) * hyb(k, in1[2]), 0)
    for in2 in kd_structure:  # it puts together the Kronecker delta and the helical term: H(k,i,j)*kd(i,l) = H(k,l,j)
...
for in1 in range(len(moznost)):  # It combines quantities with matching indices.
    ...
    Tenzor = Tenzor.subs(hyb(q, in1) * hyb(k, in1), k*q*z) # k.q = k q z, where z = cos(angle) = k . q/ |k| /|q|
    if Tenzor.coeff(hyb(p, in1)) != 0: # H( , j, s) hyb( ,j) hyb( ,s) hyb( , indexb) hyb(p, i) hyb(q, i) hyb(q, indexB) = 0 or   H( , j, indexb) hyb( ,j) hyb(p, i) hyb(q, i) hyb(q, indexB) = 0 
...
Tenzor = Tenzor.subs(hyb(q, indexb)*hyb(q, indexB), 0) # delete zero values in the Tenzor: H( ,i,j) hyb(p, i) hyb( ,j) hyb(k, indexB) hyb(k, indexb) = 0
...
while i < len(H_structure):  # I go through all of them helical term H( , , )
    in1 = H_structure[i]
    while Tenzor.coeff(H(in1[0], in1[1], in1[2])) == 0: # if the H( , , ) structure is no longer in the Tenzor, I throw it away
        H_structure.remove(in1)
        in1 = H_structure[i]  
    if in1[0] == k:  # it create a list where momenta are stored in the positions and indexes pf momenta. - I have for internal momenta k or q 
        kombinacia = in1 + [q, -1, p, -1,  k, -1, q, -1] # [ k, indexH, indexH, q, -1, p, -1,  k, -1, q, -1 ]
    else:
        kombinacia = in1 + [k, -1, p, -1,  k, -1, q, -1]
    if indexB == in1[1]:  # it looks for whether the H helicity term contains an idex corresponding to the externa field b or B
        kombinacia[4] = in1[2]
    elif indexB == in1[2]:
        kombinacia[4] = in1[1]
    elif indexb == in1[1]:
        ...
        kombinacia[4] = in1[1]
    kombinacia_old = [ kombinacia ] # search whether the index B or b is in momenta not associated with the helicity term
    kombinacia_new = list() 
    kombinacia_new.append(dosad(kombinacia_old[0], indexB, k_structure, 8)) # it create and put the field index B in to the list on the position 8: hyb(k,indexB) 
    kombinacia = dosad(kombinacia_old[0], indexB, q_structure, 10) # it create and put the field index B in to the list on the position 10: hyb(q,indexB) 
    if kombinacia not in kombinacia_new:
        kombinacia_new.append(kombinacia)
    kombinacia_old = kombinacia_new
    kombinacia_new = list()
    for in2 in kombinacia_old: # # it create and put the field index b in to the list with index  
    ...
    kombinacia_old = kombinacia_new
    kombinacia_new = list() #  I know which indexes are free. I know where the fields B or b are located. 
    for in2 in kombinacia_old: # I have free two indecies and I start summing in the tensor structure
        if in2[4] == -1 and in2[0] == k:  # it calculate if there is H(k,...,...) and the indecies of the external fields are outside
            if in2[1] in p_structure and in2[2] in q_structure: # H(k, i, j) hyb(q, j) hyb(p, i) hyb(k, indexb) hyb(q, indexB) = ... or  H(k, i, j) hyb(q, j) hyb(p, i) hyb(k, indexB) hyb(q, indexb)
                Tenzor = Tenzor.subs( H(k,in2[1],in2[2]) * hyb(q,in2[2]) * hyb(p,in2[1]) * hyb(k,in2[8]) * hyb(q,in2[10]), hyb(p, s) * lcs(s, in2[10], in2[8]) * q**2*k*(1-z**2)/d/(d+2)) 
            if in2[2] in p_structure and in2[1] in q_structure: 
                Tenzor = Tenzor.subs( H(k,in2[1],in2[2]) * hyb(q,in2[1]) * hyb(p,in2[2]) * hyb(k,in2[8]) * hyb(q,in2[10]), - hyb(p, s) * lcs(s, in2[10], in2[8])* q**2*k*(1-z**2)/d/(d+2)) 
        if in2[4] == -1 and in2[0] == q:  # 
            if in2[1] in p_structure and in2[2] in k_structure:
                Tenzor = Tenzor.subs( H(q,in2[1],in2[2]) * hyb(k,in2[2]) * hyb(p,in2[1]) * hyb(k,in2[8]) * hyb(q,in2[10]),  - hyb(p, s) * lcs(s, in2[10], in2[8]) * q*k**2*(1-z**2)/d/(d+2)) 
            if in2[2] in p_structure and in2[1] in k_structure:
                Tenzor = Tenzor.subs( H(q,in2[1],in2[2]) * hyb(k,in2[1]) * hyb(p,in2[2]) * hyb(k,in2[8]) * hyb(q,in2[10]), hyb(p, s) * lcs(s, in2[10], in2[8])* q*k**2*(1-z**2)/d/(d+2))
        if in2[8] == -1 and in2[0] == k: # # H(k, indexb, j) hyb(q, j) hyb(p, i) hyb(k, i) hyb(q, indexB) = ... or  H(k, indexB, j) hyb(q, j) hyb(p, i) hyb(k, i) hyb(q, indexb)
            for in3 in p_structure:
                if in2[1] == indexb or in2[1] == indexB:
                    Tenzor = Tenzor.subs( H(k, in2[1], in2[2]) * hyb(q,in2[2]) * hyb(p,in3) * hyb(k,in3) * hyb(q,in2[10]), - hyb(p, s) * lcs(s, in2[10], in2[1])* q**2*k*(1-z**2)/d/(d+2))
                if in2[2] == indexb or in2[2] == indexB:
                    Tenzor = Tenzor.subs( H(k, in2[1], in2[2]) * hyb(q,in2[1]) * hyb(p,in3) * hyb(k,in3) * hyb(q,in2[10]), hyb(p,s) * lcs(s, in2[10], in2[2])* q**2*k*(1-z**2)/d/(d+2))
        if in2[8] == -1 and in2[0] == q: 
            for in3 in p_structure:
                if in2[1] == indexb or in2[1] == indexB:
                    Tenzor = Tenzor.subs( H(q, in2[1], in2[2]) * hyb(k,in2[2]) * hyb(p,in3) * hyb(k,in3) * hyb(q,in2[10]), hyb(p,s) * lcs(s, in2[10], in2[1])* q*k**2*(1-z**2)/d/(d+2))
                if in2[2] == indexb or in2[2] == indexB:
                    Tenzor = Tenzor.subs( H(q, in2[1], in2[2]) * hyb(k,in2[1]) * hyb(p,in3) * hyb(k,in3) * hyb(q,in2[10]), - hyb(p,s) * lcs(s, in2[10], in2[2])* q*k**2*(1-z**2)/d/(d+2))
        if in2[10] == -1 and in2[0] == k:
            for in3 in p_structure:
                if in2[1] == indexb or in2[1] == indexB:
                    Tenzor = Tenzor.subs( H(k, in2[1], in2[2]) * hyb(q,in2[2]) * hyb(p,in3) * hyb(k,in2[8]) * hyb(q,in3), - hyb(p,s) * lcs(s, in2[1], in2[8]) *q**2*k*(1-z**2)/d/(d+2))
                if in2[2] == indexb or in2[2] == indexB:
                    Tenzor = Tenzor.subs( H(k, in2[1], in2[2]) * hyb(q,in2[1]) * hyb(p,in3) * hyb(k,in2[8]) * hyb(q,in3), hyb(p,s) * lcs(s, in2[2], in2[8]) *q**2*k*(1-z**2)/d/(d+2)) 
        if in2[10] == -1 and in2[0] == q:
            for in3 in p_structure:
                if in2[1] == indexb or in2[1] == indexB:
                    Tenzor = Tenzor.subs( H(q, in2[1], in2[2]) * hyb(k,in2[2]) * hyb(k,in2[8]) * hyb(p,in3) * hyb(q,in3),  hyb(p,s) * lcs(s, in2[1], in2[8]) *q*k**2*(1-z**2)/d/(d+2))
                if in2[2] == indexb or in2[2] == indexB:
                    Tenzor = Tenzor.subs( H(q, in2[1], in2[2]) * hyb(k,in2[1]) * hyb(k,in2[8]) * hyb(p,in3) * hyb(q,in3), - hyb(p,s) * lcs(s, in2[2], in2[8]) *q*k**2*(1-z**2)/d/(d+2))
    i += 1
...
for in1 in H_structure:  # calculate the structure where there are two external momentums: H(momentum, i, indexB)* p(i) hyb( , indexb) and other combinations except H(momentum, indexB, indexb) hyb(p, i) hyb(k, i)
    if Tenzor.coeff(H(in1[0], in1[1], in1[2])) != 0:
        if in1[1] in p_structure and in1[2] == indexb:
            if in1[0] == k:
                Tenzor = Tenzor.subs(H(k, in1[1], in1[2]) * hyb(p, in1[1]) * hyb(k, indexB), hyb(p, s) * lcs(s, indexb, indexB)*k/d ) 
                Tenzor = Tenzor.subs(H(k, in1[1], in1[2]) * hyb(p, in1[1]) * hyb(q, indexB), hyb(p, s) * lcs(s, indexb, indexB)*q*z/d )
            else:
                Tenzor = Tenzor.subs(H(q, in1[1], in1[2]) * hyb(p, in1[1]) * hyb(q, indexB), hyb(p, s) * lcs(s, indexb, indexB)*q/d ) 
                Tenzor = Tenzor.subs(H(q, in1[1], in1[2]) * hyb(p, in1[1]) * hyb(k, indexB), hyb(p, s) * lcs(s, indexb, indexB)*k*z/d )
        if in1[2] in p_structure and in1[1] == indexb:
            if in1[0] == k:
                Tenzor = Tenzor.subs(H(k, in1[1], in1[2]) * hyb(p, in1[2]) * hyb(k, indexB), - hyb(p, s) * lcs(s, indexb, indexB)*k/d ) 
                Tenzor = Tenzor.subs(H(k, in1[1], in1[2]) * hyb(p, in1[2]) * hyb(q, indexB), - hyb(p, s) * lcs(s, indexb, indexB)*q*z/d )
            else:
                Tenzor = Tenzor.subs(H(q, in1[1], in1[2]) * hyb(p, in1[2]) * hyb(q, indexB), - hyb(p, s) * lcs(s, indexb, indexB)*q/d ) 
                Tenzor = Tenzor.subs(H(q, in1[1], in1[2]) * hyb(p, in1[2]) * hyb(k, indexB), - hyb(p, s) * lcs(s, indexb, indexB)*k*z/d )
        if in1[1] in p_structure and in1[2] == indexB:
            if in1[0] == k:
                Tenzor = Tenzor.subs(H(k, in1[1], in1[2]) * hyb(p, in1[1]) * hyb(k, indexb), - hyb(p, s) * lcs(s, indexb, indexB)*k/d ) 
                Tenzor = Tenzor.subs(H(k, in1[1], in1[2]) * hyb(p, in1[1]) * hyb(q, indexb), - hyb(p, s) * lcs(s, indexb, indexB)*q*z/d )
